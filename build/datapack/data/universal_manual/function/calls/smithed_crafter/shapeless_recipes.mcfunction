...
execute if score @s smithed.data matches 0 store result score @s smithed.data if score count smithed.data matches 1 if data storage smithed.crafter:input {"recipe": [{"count": 1, "components": {"minecraft:custom_data": {"universal_manual": {"universal_manual": true}}}}]} run loot replace block ~ ~ ~ container.16 loot universal_manual:i/universal_manual
